...
import { ArrowRight, Camera, CheckCircle2, ChevronRight, Flame, Languages, RotateCcw, Send, ShieldCheck, Sparkles, Star, Trophy, UserRound, AlertTriangle, History, Square, Target, LogOut } from 'lucide-react'
...
import { getCurrentUser, getUserInitials, logout } from '../services/authService'
...
export function Profile() { const navigate = useNavigate(); const user = getCurrentUser(); const userName = user?.name || 'User'; const [progress] = useLearningProgress(); const average = progress.quizScores.length ? Math.round(progress.quizScores.reduce((sum, score) => sum + score, 0) / progress.quizScores.length) : 0; const streak = getLearningStreak(progress.practiceDates); const handleLogout = () => { logout(); navigate('/login') }; return <><PageHeader eyebrow="Your space" title="Profile" description="Make your learning experience feel like yours." /><div className="profile-layout"><section className="panel profile-card"><div className="profile-avatar">{getUserInitials(userName)}</div><h2>{userName}</h2><p>{user?.email || 'Learning ISL'} · Learning ISL since August 2024</p><span className="level-pill"><Sparkles size={14} /> Curious beginner</span><div className="profile-divider" /><div className="profile-mini-stats"><div><strong>{progress.completedSigns.length}</strong><span>Signs learned</span></div><div><strong>{streak}</strong><span>Day streak</span></div><div><strong>{average ? `${average}%` : '—'}</strong><span>Quiz average</span></div></div><div style={{ display: 'flex', gap: '8px', width: '100%', justifyContent: 'center' }}><Button variant="outline" icon={UserRound}>Edit profile</Button><Button variant="outline" icon={LogOut} onClick={handleLogout}>Sign out</Button></div></section><section className="panel achievements"><div className="section-heading"><div><span className="eyebrow">Milestones</span><h2>Achievements</h2></div><span className="muted-text">3 of 8 unlocked</span></div><div className="achievement-grid">{[['🌱', 'First steps', 'Learn your first sign', true], ['🔥', 'On a roll', 'Practice 7 days in a row', true], ['💬', 'Conversation starter', 'Learn 50 signs', false], ['🏆', 'Quiz whiz', 'Score 100% on a quiz', false]].map(([emoji, title, text, unlocked]) => <div className={`achievement ${!unlocked ? 'locked' : ''}`} key={title}><span>{emoji}</span><div><strong>{title}</strong><p>{text}</p></div>{unlocked && <CheckCircle2 size={17} />}</div>)}</div></section></div></> }
