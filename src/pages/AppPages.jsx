...
const progressKey = 'signbridge-learning-progress'
...
function readProgress() { try { return { ...defaultProgress, ...JSON.parse(localStorage.getItem(progressKey) || '{}') } } catch { return defaultProgress } }
function writeProgress(update) { const next = { ...readProgress(), ...update }; localStorage.setItem(progressKey, JSON.stringify(next)); return next }
...
export function Home() {
  return <><PageHeader eyebrow="Wednesday, 14 August 2024" title="Good morning, Aisha" description="Keep your hands moving. You’re building something meaningful." action={<Button icon={ArrowRight}>Continue learning</Button>} /><div className="hero-banner"><div><span className="eyebrow">Your next step</span><h2>Everyday greetings</h2><p>Pick up where you left off: learn six signs for warmer, easier conversations.</p><Button variant="dark" icon={PlayIcon}>Resume lesson</Button></div><div className="hero-art"><span>👋</span><span>✦</span></div></div><div className="stats-grid"><StatCard icon={Sparkles} label="Signs learned" value="24" detail="+4 this week" /><StatCard icon={Flame} label="Practice streak" value="7 days" detail="Personal best: 12" tone="gold" /><StatCard icon={Trophy} label="Quiz score" value="86%" detail="Top 18% this week" tone="blue" /></div><div className="dashboard-grid"><section className="panel lessons-panel"><div className="section-heading"><div><span className="eyebrow">Keep going</span><h2>Recent lessons</h2></div><a href="/learn">View all <ArrowRight size={15} /></a></div>{lessons.map((lesson) => <LessonRow key={lesson.title} lesson={lesson} />)}</section><section className="panel daily-panel"><span className="eyebrow">Daily practice</span><h2>A little practice<br /><em>goes a long way.</em></h2><p>Three minutes today keeps your progress moving forward.</p><div className="daily-ring"><strong>3</strong><span>min</span></div><Button variant="outline" icon={ArrowRight}>Practice now</Button></section></div></>
}
